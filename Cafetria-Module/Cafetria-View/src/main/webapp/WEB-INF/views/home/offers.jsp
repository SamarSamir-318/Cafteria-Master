<%-- 
    Document   : allOffers
    Created on : Jun 15, 2017, 2:04:50 PM
    Author     : atom
--%>
...
<div class="button-order" onclick="selectedItems()">Take Offer</div>
<c:forEach items="${offerList}" var="offer">
	<div class="col-md-4" style="margin-bottom: 2%;">
		<div class="food">
			<div class="cover img-responsive" style="background-image:url('${cntxt}/resources/sp_off.png');background-size: contain;">	
				<label>
					<i>
				...
				<span class="order-selection"><input id="${offer.id}" type="checkbox" class="checkboxes" /> </span>
			<div class="info">
				<a href="#" class="recipe" class="btn btn-info btn-lg" data-toggle="modal" data-target="#detailsModal-${offer.id}">
					<i>
					...
					<table width="100%" cellpadding="0" cellspacing="0">
						<tr>
							<td width="140" rowspan="2" align="center" valign="middle">
								<h2>${offer.discountPercentage}%</h2>
							</td>
						</tr>
	...
	<div id="detailsModal-${offer.id}" class="modal fade" role="dialog">
		<div class="modal-dialog">
			<!-- Modal content-->
			<div class="modal-content">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal">Items</button>
					<h4 class="modal-title">What items?!</h4>
				</div>
				<div class="modal-body">
					<i>Description: ${offer.description}</i>
					<c:forEach items="${offer.itemCollection}" var="item">
						<p><strong>${item.name}: </strong>  <del>${item.price}</del> - <ins>${item.price * ( 1 - offer.discountPercentage / 100)}</ins>></p>
					</c:forEach>
					<h5>from: ${offer.startTime}, to: ${offer.startTime}</h5>
				</div>
...
<a href="success" hidden="true" id="addbutton"> </a>

<script>
    function addOffer(row) {
        $.ajax({
            url: "/Cafetria/offerOrder?id=" + row
            ,
            type: 'GET'
            ,
